...
func (db *ErmineDB) setTTL(dType DataType, key string, ttl int64) {
	db.exps.HSet(dType, key, ttl)
}

func (db *ErmineDB) getTTL(dType DataType, key string) interface{} {
	return db.exps.HGet(dType, key)
}

func (db *ErmineDB) hasExpired(key string, dType DataType) (expired bool) {
...
func (db *ErmineDB) evict(key string, dType DataType) {
	ttl := db.exps.HGet(dType, key)
	if ttl == nil {
		return
	}
...
func (db *ErmineDB) write(r *record) error {
	if db.log == nil {
		return nil
	}
	encVal, err := r.encode()
