...
var (
	dbLogPrefix    = []byte(ELOGPREFIX) // Bucket names we perform transactions in
	dbDatPrefix    = []byte(EDATPREFIX)
	dbU64Prefix    = []byte(EU64PREFIX)
	dbSstPrefix    = []byte(ESSTPREFIX)
	ErrKeyNotFound = errors.New("not found")
	log            = hclog.New(&hclog.LoggerOptions{Name: "erminedb"})
)

/*
	ErmineStore provides access to ErmineDB for Raft to store and retrieve log entries.
...
*/
type ErmineStore struct {
	db *ErmineDB
}

func NewErmineStore(path string) (*ErmineStore, error) {
	config := &Config{Path: path, EvictionInterval: 120, NoSync: true}
	// config := &erminedb.Config{Path: "", EvictionInterval: 10}
	zlog.Info().Msgf("path: %d", path)
	db, err := New(config)
	if err != nil {
		log.Error("Failed to create ErmineDB")
	}
	store := &ErmineStore{db: db}
	return store, nil
}
...
func sstKeyOf(rawKey []byte) []byte {
	...
	if log.IsTrace() {
		log.Trace("ermine key", "sst", key)
	}
	return []byte(key)
}

func (b *ErmineStore) generateRanges(min, max uint64, batchSize int64) []IteratorRange {
...
func (b *ErmineStore) FirstIndex() (uint64, error) {
	return b.db.strStore.firstIndex(string(dbLogPrefix)), nil
}

// LastIndex returns the last known index from the Raft log.
func (b *ErmineStore) LastIndex() (uint64, error) {
	return b.db.strStore.lastIndex(string(dbLogPrefix)), nil
}

// GetLog is used to retrieve a log from Ermine at a given index.
func (b *ErmineStore) GetLog(idx uint64, log *raft.Log) error {
	...
	item, _ := b.db.strStore.get(string(logKeyOf(idx)))
	if item == nil {
		return raft.ErrLogNotFound
	}
	val := []byte(item.(string))
	buf := bytes.NewBuffer(val)
	dec := gob.NewDecoder(buf)
	err := dec.Decode(&log)
	return err
...
func (b *ErmineStore) StoreLogs(logs []*raft.Log) error {
	...
	for _, r := range ranges {
		for index := r.from; index < r.to; index++ {
			...
			enc := gob.NewEncoder(&out)
			enc.Encode(log)
			err := b.Set(logKeyOf(log.Index), out.Bytes())
			if err != nil {
				return err
			}
		}
...
func (b *ErmineStore) StoreLog(log *raft.Log) error {
	return b.StoreLogs([]*raft.Log{log})
}

// DeleteRange is used to delete logs within a given range inclusively.
func (b *ErmineStore) DeleteRange(min, max uint64) error {
	// maxBatchSize := b.Db.MaxBatchSize()
	// ranges := b.generateRanges(min, max, maxBatchSize)
	// for _, r := range ranges {
	// 	txn := b.Db.NewTransaction(true)
	...
	for _, r := range ranges {
		for index := r.from; index < r.to; index++ {
			delKey := logKeyOf(index)

			err := b.db.Update(func(tx *Tx) error {
				err := tx.Delete(string(delKey))
				return err
			})
			if err != nil {
				return err
...
func (b *ErmineStore) GetRaw(k []byte) ([]byte, error) {
...
func (b *ErmineStore) SetRaw(k []byte, v []byte) error {
	// return b.Db.Update(func(txn *badger.Txn) error {
	// 	return txn.Set(k, v)
	// })
	err := b.db.Update(func(tx *Tx) error {
		err := tx.Set(string(k), string(v))
		return err
	})
	if err != nil {
		return err
...
func (b *ErmineStore) DeleteRaw(key []byte) error {
	// return b.Db.Update(func(txn *badger.Txn) error {
...
func (b *ErmineStore) KeysOf(prefix []byte, offset []byte, pageSize uint16) (*VpKeyPage, error) {
	// keys := make([]string, 0)
...
func (b *ErmineStore) GetData(key []byte) ([]byte, error) {
...
func (b *ErmineStore) Get(k []byte) ([]byte, error) {
...
func (b *ErmineStore) Set(k []byte, v []byte) error {
	return b.SetRaw(sstKeyOf(k), v)
}

// SetUint64 is like Set, but handles uint64 values
func (b *ErmineStore) SetUint64(key []byte, val uint64) error {
	return b.SetRaw(u64KeyOf(key), uint64ToBytes(val))
}

// GetUint64 is like Get, but handles uint64 values
func (b *ErmineStore) GetUint64(key []byte) (uint64, error) {
...
func (b *ErmineStore) Apply(rLog *raft.Log) interface{} {
	switch rLog.Type {
...
func (b *ErmineStore) Close() error {
	// return b.Db.Close()
	return b.db.Close()
}

func (b *ErmineStore) Snapshot() (raft.FSMSnapshot, error) {
	return &Snapshot{}, nil
}

func (b *ErmineStore) Restore(rc io.ReadCloser) error {
	return nil
}
...
func (b *ErmineStore) Keys(bucketName []byte) []string {
	// var keys []string
	...
	// 		// 			k, _ := hex.DecodeString(rk)
	// 		// 			itKey := string(k)
	// 		//             log.Info("This key", "key", itKey)
	// 		//             keys = append(keys, itKey)
	// 	}
	// 	return nil
	// })
	// if err != nil {
